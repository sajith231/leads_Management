...
def save_ratings(request, cv_id):
    if request.method == 'POST':
        try:
            ...
            rating.knowledge = request.POST.get('knowledgeRating')
            rating.confidence = request.POST.get('confidenceRating')
            rating.attitude = request.POST.get('attitudeRating')
            rating.communication = request.POST.get('communicationRating')
            rating.appearance = request.POST.get('appearanceRating')
            rating.languages = request.POST.getlist('languages[]')
            rating.expected_salary = request.POST.get('expectedSalary') or None
            rating.experience = request.POST.get('experience') or None
            rating.remark = request.POST.get('remark') or None

            # Set the user who added the ratings
            rating.created_by = request.user  # Add this line

            # Handle voice note
            voice_note_data = request.POST.get('voice_note')
            if voice_note_data:
                # Save the voice note (implementation depends on your file handling logic)
                pass
        ...
        try:
            ...
            if rating:
                response_data = {
                    ...
                    'username': rating.created_by.username if rating.created_by else 'Unknown'  # Add this line
                }
